...
def _strip_nii_gz(path: str) -> str:
...
def add_data_dir2path(list_files: list, data_dir: str, fold: int = None, args = None, index = None, phase = None) -> tuple[list, list]:
    """
    ...
    if index is not None:
        new_list_files = new_list_files[index:index+100]
    if fold is not None:
    ...
    for d in new_list_files:
        rel_image = d["image"]
        d["image"] = rel_image if os.path.isabs(rel_image) else os.path.join(data_dir, rel_image)

        if "label" in d:
            rel_label = d["label"]
            d["label"] = rel_label if os.path.isabs(rel_label) else os.path.join(data_dir, rel_label)
        else:
            d["label"] = d["image"].replace("dataset/", "dataset/ts_seg/ts_total/").replace("train/", "train_fixed/").replace("valid/", "valid_fixed/")

        if args.embedding_base_dir is not None:
            image_abs = os.path.abspath(d["image"])
            rel_to_data = os.path.relpath(image_abs, data_dir_abs)
            emb_base = os.path.join(args.embedding_base_dir, _strip_nii_gz(rel_to_data))
            d["image"] = emb_base + "_emb.nii.gz"
            str_info = emb_base + ".json"
            str_info = str_info.replace("_emb", "")
            str_cond = str_info.replace(".nii.gz.json", f"_impression_{args.report_encoder_model}.npy")
            d["top_region_index"] = str_info 
            d["bottom_region_index"] = str_info
            d["spacing"] = str_info
            d["cond"] = str_cond
            d["filename"] = str_info.replace('.json', '')
...
def prepare_maisi_controlnet_json_dataloader(
    ...
    if isinstance(json_data_list, list):
        ...
        for data_list, data_root in zip(json_data_list, data_base_dir):
            with open(data_list, "r") as f:
                json_data = json.load(f)[phase]
            train, val = add_data_dir2path(json_data, data_root, fold, phase)
            list_train += train
            list_valid += val
    else:
        with open(json_data_list, "r") as f:
            json_data = json.load(f)[phase]
        list_train, list_valid = add_data_dir2path(json_data, data_base_dir, fold, args, index, phase)
    ...
    common_transform = [
        ...
        Lambdad(keys="top_region_index", func=lambda x: _load_data_from_file(x, "top_region_index"), allow_missing_keys=True),
        Lambdad(keys="bottom_region_index", func=lambda x: _load_data_from_file(x, "bottom_region_index"), allow_missing_keys=True),
        Lambdad(keys="spacing", func=lambda x: _load_data_from_file(x, "spacing"), allow_missing_keys=True),
        Lambdad(keys=["top_region_index", "bottom_region_index", "spacing"], func=lambda x: x * 1e2, allow_missing_keys=True),
        Lambdad(keys="cond", func=lambda x: _load_npy(x)),
    ]
