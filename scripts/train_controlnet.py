...
def main():
    ...
    parser.add_argument(
        "-e",
        "--environment-file",
        default="./configs/environment_rag_controlnet_train.json",
    ...
    if args.trained_controlnet_path is not None:
        ...
        controlnet.load_state_dict(_extract_state_dict(torch.load(args.trained_controlnet_path, map_location=device, weights_only=False)), strict=False)
    ...
    for epoch in range(n_epochs):
        ...
        for step, batch in enumerate(train_loader):
            ...
            with autocast("cuda", enabled=True):
                mask_uncond = torch.rand(images.shape[0], device=images.device) < args.controlnet_train['conditional_free_guidance']
                cond_masked = cond.clone()  # Copiamo il tensore condizionante
                ...
                if isinstance(noise_scheduler, RFlowScheduler):
                    ...
                    timesteps = torch.randint(
                        0, noise_scheduler.num_train_timesteps, (images.shape[0],), device=images.device).long()
