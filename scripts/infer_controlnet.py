...
def main():
    ...
    parser.add_argument("-i", "--index", default=0, type=int, help="number of samples")
    ...
    for num in range(len(iterator)):
        try:
            ...
            guidance_scale = args.controlnet_infer['guidance_scale'] if use_cfg else 1.0  # Usa il valore di CFG se attivo
            ...
            output_path = "{0}/{1}{2}".format(
                args.output_dir,
                args.model_dir.replace('./models/', '') + '_' + str(args.controlnet_infer['guidance_scale']),
                output_prefix,
            )
            ...
            label_saver = SaveImage(
                output_dir= os.path.dirname(output_path),
                output_postfix=os.path.basename(output_path).replace(os.path.basename(output_path), '_label'),
                separate_folder=False
            )
            label_saver(labels)
            """
        except Exception as e:
            print(f"Skipping sample {num} due to error: {e}")
    if use_ddp:
