    ...
    with torch.no_grad(), torch.amp.autocast("cuda"):
        ...
        for t, next_t in progress_bar:
            if use_cfg:
                ...
                controlnet_inputs_no_txt = copy.deepcopy(controlnet_inputs)
                controlnet_inputs.update(
                    {
                        "context": cond,
                    }
                    )
                controlnet_inputs_no_txt.update(
                    {
                        "context": torch.zeros_like(cond, device=device),
                    }
                    )

                # ControlNet pass senza condizione (usando tutto a zero come condizionamento)
                down_block_res_samples_uncond, mid_block_res_sample_uncond = controlnet(
                    **controlnet_inputs_no_txt
                )
                # ControlNet pass con condizione
                ...
                unet_inputs_no_txt = copy.deepcopy(unet_inputs)
                unet_inputs.update(
                    {
                ...
                # UNet pass senza condizione
                noise_pred_uncond = diffusion_unet(
                    **unet_inputs_no_txt
                )

                # UNet pass con condizione
                noise_pred_cond = diffusion_unet(
                    **unet_inputs
                )
        ...
        if use_cfg:
            del (
                down_block_res_samples_uncond,
                down_block_res_samples_cond,
                mid_block_res_sample_uncond,
                mid_block_res_sample_cond
            )
        else:
            del (
        ...
        # synthetic_image = crop_img_body_mask(synthetic_images, combine_label)
        torch.cuda.empty_cache()
